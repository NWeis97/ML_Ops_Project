...
class ConvolutionModel_v1(nn.Module):
    ...
    def __init__(self):
        ...
        # self.cl4 = nn.Conv2d(in_channels=8, out_channels=8,
        #                       kernel_size=3, stride=1, padding=1) #3->1
...
class CNNModuleVar(nn.Module):
    ...
    def __init__(
        ...
        # self.non_linear_functions_conv =
        #           [x() for x in non_linear_function_array[:self.hidden_conv_layers]]
        # self.non_linear_functions_linear =
        #               [x() for x in non_linear_function_array[self.hidden_conv_layers:]]
        # Extract reguralizers - not implemented yet
        # self.regularization_conv =
        #           [x() for x in regularization_array[:self.hidden_conv_layers]]
        # self.regularization_linear =
        #               [x() for x in regularization_array[self.hidden_conv_layers:]]

        # Put convolutions in list
        for layer in range(self.hidden_conv_layers):
            self.conv_functions.append(
                nn.Conv2d(
                    in_channels=input_channel,
                    out_channels=hidden_channel_array[layer],
                    kernel_size=hidden_kernel_array[layer],
                    stride=hidden_stride_array[layer],
                    padding=hidden_padding_array[layer],
                )
            )
            input_channel = hidden_channel_array[layer]
        self.conv_functions = nn.ModuleList(self.conv_functions)

        # Put fully connected layers in list
        input_dim = conv_to_linear_dim
        for layer in range(self.hidden_linear_layers):
            self.linear_functions.append(nn.Linear(input_dim, hidden_dim_array[layer]))
            input_dim = hidden_dim_array[layer]
